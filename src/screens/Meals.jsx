...
export function MealsScreen({ openDish, openSearch, favs, setFavs }) {
  const { route, go } = useUser();
  const cat = route.cat;
  ...
  return (
    <div className="pb-4">
      <div className="sticky top-0 z-10 px-5 pt-4 pb-3" style={{ background: C.warm }}>
        ...
        <button type="button" onClick={openSearch} className="flex items-center gap-2 rounded-full px-4 py-3 w-full mt-3 text-left" style={{ background: '#fff', border: `1px solid ${C.line}` }}>
        ...
        <div className="rounded-2xl px-4 py-2.5 mt-3 text-xs font-medium" style={{ background: '#FDF3E7', color: '#b06c22', border: '1px solid #F8DCB8' }}>
          20% off your first week with code NOURISH20 — mention it on WhatsApp
        </div>
        <div className="flex gap-2 mt-3 overflow-x-auto pb-1 no-scrollbar">
          {MENU_CHIPS.map((c) => (
            <button key={c} type="button" aria-pressed={cat === c} onClick={() => go('meals', cat === c ? null : c)} className="flex-none text-xs font-medium px-3.5 py-2 rounded-full transition-all"
        ...
        <div className="flex gap-1.5 mt-2.5" role="group" aria-label="Diet filter">
          {['All', 'Veg', 'Non-Veg', 'Vegan'].map((d) => (
            <button key={d} type="button" aria-pressed={diet === d} onClick={() => setDiet(d)} className="flex-1 text-xs font-medium py-2 rounded-full"
      ...
      <div className="px-5 grid gap-4 mt-2">
        <div className="text-xs" role="status" style={{ color: C.mute }}>{filtered.length} meals{cat ? ` · ${cat}` : ''}</div>
